...
(i.e. the waiting distance until a genealogy change). 
...
by constraining the number and identity of available lineages and by dictating 
...
lengths of the underlying genealogies. By preventing coalescence between select lineages, 
species tree divergence events enforce minimum branch lengths for the genealogies, 
increasing the number of recombination events per base pair. Similarly, variation in Ne 
affects the per-base-pair recombination rate, since coalescences farther back in time 
increase the total branch lengths of the genealogies.
Topological and sampling parameters can also affect the expected distances until genealogy 
changes -- including tree shape, tree size (i.e. number of tips), and the distribution of samples among species tree 
lineages. These factors all exert their influence through the same general mechanisms: affecting the 
probability of coalescence among certain lineages by constraining the availability of those lineages 
for coalescence with each other in each segment of the species tree, and by influencing the 
rate of recombination by affecting the total branch length of the genealogies. 
...
are useful for estimating the \emph{mean} linkage across all genealogies under a 
specific demographic model, they are less helpful for estimating the 
persistence of individual genealogies. To generate an approximate distribution 
of waiting distances locally on a chromosome using simulation methods, 
simulations generating a full probability distribution would have to be 
run individually for each genealogy in a sequence. 
This is not a tractable solution across long sequences of genealogies. 
...
high spatial variance in the expected waiting distances until genealogical 
topology changes (e.g. Fig.~\ref{fig:fig-validation}). 
Until now, there has been no analytical approach for generating a null hypothesis 
concerning the expected length of regions spanned by a single genealogical topology under absolute neutrality.
Our results allow us to calculate these expectations and to probe null hypotheses concerning the probability 
of observing specific windows representing a particular genealogy under arbitrary demographic models. 
While the MSC allows us to calculate the probability of observing a particular 
genealogy in the first place, the equations here represent a further step by 
generating a density function that represents the likelihood of \emph{persistence} 
of that topology across some region of the genome, under neutrality alone. 
...
\subsubsection{Extending SMC' waiting distance solutions:}
...
Finally, following the approach described in equations 7-10, we can calculate 
an exponential probability distribution for waiting distances between 
no-change or tree-change events using rate parameters that are 
scaled by the probabilities of either event type.
