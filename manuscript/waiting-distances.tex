...
\begin{figure}
	...
	\caption{
		...
		A recombination event can occur on any branch of a genealogy to cause one
		of three possible outcomes (b-d): "no-change", "tree-change", or 
		"topology-change", and each branch exhibits piecewise variation in these 
...
% (i.e., "waiting distances").
 % between genealogies).
and later extended as a spatial algorithm for stochastically generating full 
...
despite the occurrence of a recombination even between them. The inclusion of
...
can be associated with a different $N_b$ value and an increase in the number 
of samples.
% to the next can be 
...
or $N_b$ and $W$ in MSC models \citep{rannala2003bayes}, 
...
species tree branches into ancestral intervals increases $k$, and $N_b$ 
...
Some additional indexing variables are also described below, and
a summary of all variables is available in table~\ref{tab:tab2}). 
...
or topology. 
The interval in which recombination occurs at time $t_r$ on branch $b$ is 
labeled $i$. Equation 3 describes the probability of a no-change event 
given a genealogy embedded in a species tree, and the timing and branch on 
which recombination occurs. The first two terms describe the probability 
that the subtree both detaches and re-attaches to the same branch during 
interval $i$ (i.e., $ii$), while the last term describes the probability 
that it detaches in $i$ and re-coalesces in a later interval on branch $b$
(i.e., $ij$). For this latter term, we define an indexing variable 
$\mathcal{J}_b(i)$ = $\{j \in \mathcal{I}_b ~|~ j > i\}$, for iterating 
over the ordered intervals above $i$ on branch $b$ (e.g., Fig.~\ref{fig:fig3}b).
...
\begin{figure}[t]
	...
	\caption{
		...
		re-coalescing in the intervals between $i$ and $j$, which is described
		as $\mathcal{Q}_b(i,j)$. 
	}
	\label{fig:fig3}
\end{figure}
...
\noindent This includes the following function, $f(i,j)$, which 
...
can occur we define several additional variables. The lowest interval in which both 
$b$ and $b'$ are present and exist within the same species tree interval is labeled 
$m$, and the lower boundary of this interval as $t_b^m$. For a branch $b$ with 
intervals $\mathcal{I}_b$, the subset of intervals below $m$ is 
as $\mathcal{L}_b$, and the subset including $m$ and above is $\mathcal{M}_b$. 
...
probabilities where recombination occurs in interval $i$ and re-coalescence
...
re-examine this potential bias with respect to the presence
of population structure in our validations below.
...
and validate our solutions against expectations from coalescent simulations 
...
interval lengths X = ($x_1, x_2, ..., x_n$), a subset of genealogies 
can be extracted representing each tree-change event, e.g., 
$G_g$ = ($\mathcal{G}_1, \mathcal{G}_i, ...$). The lengths of intervals 
between these events can be summed as tree-change waiting distances
...
exponential rate parameters $\Lambda_g = (\lambda_1, \lambda_i, ...)$
from equations 10 or 14. 
The likelihood of each waiting distance (X$_g$) between tree or topology change
events can then be calculated from each exponential probability density
function (equation 8) parameterized by the sequence of rate parameters 
($\Lambda_g$). The set of species tree parameters that maximize the 
summed log-likelihood of all observed distances between events is the 
...
We also inferred a likelihood surface for a misspecified model, representing a 
single population with constant $N_e$. In this case, the $\Lambda_g$ rate 
parameters are calculated without enforcing a population split.
This clearly introduces a significant bias in estimation, shifting the 
likelihood surface towards lower $r$ and higher $N_e$. This demonstrates that even 
...
\hl{Add paragraph about variable $N_e$ model.}
\begin{figure}[t]
	\centering
	%\fbox{\rule[-.5cm]{4cm}{4cm} \rule[-.5cm]{4cm}{0cm}}
	\includegraphics[width=0.55\textwidth]{figures/likelihood-posteriors.pdf}
	\caption{
		MS-SMC' likelihood framework. 
		ARGs were simulated under a two-population species tree model with variable 
		$N_e$. (e) A joint posterior distribution of $N_e$ parameters inferred 
		by Bayesian MCMC from tree and topology-change waiting distances.
	}
...
\begin{figure}[t]
	...
	\caption{
		MS-SMC' likelihood framework. 
		(a) ARGs were simulated under a two-population species tree model with variable 
		$N_e$. (b) A joint posterior distribution of $N_e$ parameters inferred 
		by Bayesian MCMC from tree and topology-change waiting distances.
	}
	\label{fig:fig-likelihood-posterior}
\end{figure}
...
and a different, arbitrary Ne value for each species tree branch. These
solutions lay a groundwork for exploring how species tree structures 
affect neutral expectations of genealogical heterogeneity across chromosomes.

% what do our results mean overall.
A primary result of our work is demonstrating the mechanistic link between species tree
%parameters and the length of genomic intervals that represent a single genealogy. 
...
through exhaustive simulations (e.g., \citep{mckenzie_multispecies_2020}. With our analytical results it is clear how 
in addition to MSC parameters of divergence times and effective population sizes, 
other factors can also clearly affect the expected distances until genealogy changes. These factors include 
tree shape, tree size, and the distribution of samples among species tree 
lineages. Therefore, the scale at which we observe neutral turnover across 
genealogies is highly dependent on the species tree model. 

% importance to phylogenetics
%By accounting for the genomic heterogeneity that is expected due to incomplete 
%lineage sorting, t
The multispecies coalescent model has facilitated the widespread 
use of multilocus data for phylogenetic inference by accounting for genomic heterogeneity. 
%However, the MSC does not consider autocorrelation along the genome due to linkage, and it assumes 
...
of the increased resolution offered by genomic data. The traditional multispecies 
coalescent model overlooks the process of recombination, assuming that loci 
represent a single genetic history and that they are completely unlinked. In 
reality, under a neutral model, species tree parameters and recombination 
rates will influence the degree of genealogical turnover along a chromosome, 
potentially resulting in linked loci and/or multiple genealogical topologies 
per locus. Therefore, not accounting for recombination might mislead summary statistics methods (CITE ASTRAL, MP-EST). 
Methods have been designed to account for this by operating directly on SNP frequencies \citep{bryant2012inferring,chifman2014quartet,vachaspati2018svdquest}, 
but we propose that incorporating the rates of turnover observed in the data as 
...
Inference of genealogies along a chromosome is a common goal in population genetics, 
and similar efforts have been undertaken in phylogenetics. Often, the goal of such 
efforts is to detect signals of introgression or selection. However, the phylogenetic 
approaches usually do not explicitly incorporate a model for recombination 
\citep[e.g.,][]{li2019recombination}. Applications of our solutions might provide 
valuable null hypotheses of genealogical turnover rates by which introgression or 
selection might be detected. Beyond its use for detecting patterns resulting from 
...
Topological gravity wells. Even in the case of a neutral coalescent process 
...
MS-SMC'. These equations, described below, use the terms defined in 
Table~\ref{tab:table-S2}. 
% What is a species tree
A species tree $\mathcal{S}$ is a parameterized multispecies coalescent model
(demographic model) in which a set of isolated populations are related by a bifurcating
...
Given a genealogy embedded in a species tree, a series of discrete intervals 
can be defined that are delimited by events that change the rate of 
coalescence. This is termed a genealogy embedding table (e.g., Table~\ref{tab:table-1}). 
...
events. Because N$_e$ is constant in their framework, only $k$ differs between 
intervals, and this determines differences in rates of coalescence, 
which decrease in subsequent intervals from the tips towards the root. 
...
species tree branch, and correspond to time intervals with a constant 
$k$ and $N_e$. Breakpoints between intervals arise where divergences 
...
defined as $\mathcal{I}_b$. The lower and upper bound of each interval is 
...
subtree re-coalesces with any other available branch above that time. 
...
\subsubsection{Probability of no-change given a branch and time of recombination}
...
re-coalesces with the same branch it detached from. Thus, we can integrate over the
length from the time of recombination ($t_r$) to the top of the branch ($t_b^u$), the 
...
\noindent To derive the expected waiting distance to the next tree change, 
we use describe the probability of a tree-change event as 
$1-\mathbb{P}(\textrm{tree-unchanged} | \mathcal{S}, \mathcal{G})$. 
Following the approach described in equations 7-10, we can calculate an 
...
% Next, we attempt to further exclude recombination events. We filter out recombination 
% events of categories 2 and 3, which just change branch lengths, to isolate the probability
% that a recombination event changes the \emph{topology} of the tree.
As in the first section, we treat branches individually and then sum across them. However,
we have to designate new variables. We now consider two lineages in addition to the 
focal lineage $b$: the lineage $b'$ that $b$ coalesces with, and the lineage $c$ that 
is ancestral to $b$ and $b'$. We also designate the timepoint $t_b^m$, which we use to 
...
\noindent Where $t^m_b=\max\{t^w_{(b,b')},t_{b'}^l,t_b^l\}$.
...
The final solution here is shown as equation XX of the main text. It is an integral 
over part 1, two times the integral in part 2, since this length exists on both branches
b and b', and then the integral over part 3. 
...
\begin{aligned}
	...
	&\frac{1}{k_i} + 
	\sum_{j \in \mathcal{I}_{bc}}	f(i,j) \exp \bigg\{	\frac{k_i}{n_i} t_r \bigg\} + 
	\sum_{j \in \mathcal{M}_b}    f(i,j) \exp \bigg\{ \frac{k_i}{n_i} t_r \bigg\}
\end{aligned}
\end{equation}

\paragraph{Second case --} Given $t \in [\sigma_i, \sigma_{i+1}] \subset [t_b^m,t_b^u]$.
This is similar to the first, but we can eliminate the first integral. The final result
is shown as equation XX in the main text.
...
\begin{aligned}
	...
	& 2 \bigg(
		\frac{1}{k_i} + 
		\sum_{k \in \mathcal{I}_b} f(i,j) \exp \bigg\{ \frac{k_i}{n_i} t_r \bigg\}
	\bigg) + 
	\sum_{j \in \mathcal{I}_c} f(i,j) \exp \bigg\{ \frac{k_i}{n_i} t_r \bigg\}
\end{aligned}
\end{equation*}
...
change the topology. We integrate across values of $t$ and sum the two cases defined above, while 
assuming a uniform probability of the recombination event occurring at any time along the branch:
...
\begin{aligned}
	...
	\sum_{i \in \mathcal{M}_b} \frac{1}{k_i} \Bigg[ 
		d_i + n_i \bigg( 
			\exp \bigg\{\frac{k_i}{n_i} \mu_i \bigg\} - 
			\exp \bigg\{\frac{k_i}{n_i} \sigma_i\bigg\} 
		\bigg)
		\bigg(
			\sum_{j \in \mathcal{I}_{bc}} f(i,j) + \sum_{j \in \mathcal{M}_b} f(i,j) 
		\bigg) 
...
\begin{equation}
\begin{aligned}
	&\int_{t_b^m}^{t_b^u} \mathbb{P} (\textrm{topology-unchanged} | \mathcal{S}, \mathcal{G}, b, t_r) = \\
	&\sum_{i \in \mathcal{M}_b} \frac{1}{k_i} \Bigg[ 
		2d_i + n_i \bigg( 
			\exp \bigg\{ \frac{k_i}{n_i} \mu_i \bigg\} - 
			\exp \bigg\{ \frac{k_i}{n_i} \sigma_i \bigg\}
...
Finally, we sum across all branches to find the probability of a recombination event changing the 
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.9\textwidth]{figures/error-smc-approx.pdf}
	\caption{
		Error in ...
	}
