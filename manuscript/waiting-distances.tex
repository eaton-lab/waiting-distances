...
events ($W$) that separate $b$ and $b'$ (Fig.~\ref{fig:figS-tbm}a-c).
...
change the topology by integrating across all values for $t_r$.
Following the approach above, we split this problem into two parts, above and below
$t_b^m$, and sum the two cases.
...
We can now sum over each entire interval below $t_b^m$ where $t_r$ could occur, and 
...
Similarly, we can sum over each entire interval above $t_b^m$ and substitute piece-wise
