...
an important component of many MSC-based methods \citep{ADD-SOME-HERE}.
...
imbalanced 4-tip species tree with \hl{constant} effective population size 
...
The exponential rate parameter ($\lambda$) for a probability density of waiting distances 
...
of constant N$_e$ values (Fig.~\ref{fig:fig-validation}b-f). 
For each value of N$_e$, 10K tree sequences were simulated. Given each starting genealogy,
...
Population structure enforces a limit on the minimum length of coalescent 
times, since genealogies must be capable of being embedded in the species 
tree topology.
This has a clear effect on $L(\mathcal{G})$ at both low and high N$_e$
(Fig.~\ref{fig:fig-validation}b): population structure shifts
the minimum and mean $L(\mathcal{G})$ higher. Because all 
recombination events occur at a rate proportional to $L(\mathcal{G})$ 
(the opportunity over which recombination can occur), larger $L(\mathcal{G})$ 
is expected to decrease the waiting distances until each recombination event 
type, all else being equal. However, all else does not remain equal. Population
structure also has the opposite effect on waiting distances by decreasing 
the probability of tree or topology changes (Fig.~\ref{fig:fig-validation}c-d), 
especially at lower N$_e$ values, where lineages are more often restricted from
re-coalescing with other lineages outside of the same species tree interval.
Consequently, the waiting distances until each event type 
(Fig.~\ref{fig:fig-validation}e-f) represent a balance of the positive and 
negative effects of population structure on $L(\mathcal{G})$ and $\mathbb{P}$, 
respectively.

% Analytical predictions are very accurate, even better given SMC' approximation.
The exact analytical solutions under the MS-SMC' for the probabilities 
of different recombination event type outcomes, and for the 
waiting distance until a tree-change event, appear to converge on the 
mean expectations from stochastic coalescent simulations 
(Fig.~\ref{fig:fig-validation}c-f).

The degree of matching between our analytical 
solutions and simulated values is slightly higher when simulations are 
performed under the assumptions of the SMC' (Fig.~\ref{fig:supplement-smc}), 
and only slightly ... exact when simulated under the full coalescent with recombination
(Fig.~\ref{fig:fig-validation}c-e), suggesting that the approximation 
assumptions inherent to the SMC' do not significantly bias estimated 
waiting distances in the structured population models examined here.
...
To investigate whether the SMC' approximation inherent to the MS-SMC' introduces
a significant bias we compared MS-SMC' predictions to tree sequences simulated
under both the SMC' (Fig.~\ref{...}) and the full coalescent with recombination 
(Fig.~\ref{xxx}). In both cases the MS-SMC' predictions appear to converge 
on the simulated expectations except in the extreme case of high population structure
and ...

The SMC' approximation to the coalescent with recombination deviates more
significantly from the full model as the number of recombination events 
that the SMC' does not model (among ancestors that do not leave sampled 
descendants) increases. Thus, the SMC' is expected to deviate most significantly
in highly structure populations with low Ne?
