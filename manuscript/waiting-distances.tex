...
were predicted based only on the embedding of the first genealogy in each tree sequence 
...
a history of recombination events between genome segments inherited from 
different ancestors. The sequentially Markov coalescent describes a generative 
process for which statistical methods can be developed to model the variation 
between sequential genealogies. Although this has led to many significant 
advances for the study of linked genome data, such methods remain highly limited 
in the scale over which they extract information from genomes -- essentially
extending forward just one recombination event at a time. 
For many datasets, this limits the information that is available for SMC-based 
inference methods, since most observations are uninformative. 
The recent development by \citep{Deng} of solutions for the probabilities 
of different categorical types of recombination events (tree-changes and 
topology-changes) addresses this problem by modeling sequential events 
that extend over greater spatial genomic distances. 
...
implementation relative to stochastic simulations performed under either the 
full coalescent with recombination or the SMC' approximation (...). Within this
framework, MSC model parameters determine rates of coalescence within 
species tree intervals, and prevent coalescence among lineages in separate
species tree intervals. For any MSC model, these effects can be statistically 
...
spatial distances over which those genealogies are expected to span given 
the MSC model.
...
a large set of sampled genealogies under a specific demographic model, it cannot
provide an estimate for the persistence of any \emph{individual} genealogy. 
% Similarly, it did not... more here please.
The analytical solutions that we derived in this study 
allow us not only to predict waiting distances for any embedded genealogy, 
but furthermore, to develop a likelihood framework for modeling observed 
waiting distances. 

% This framework can be extended...
\hl{... Many extensions of the MS-SMC' are possible. One approach could be to 
estimate the expected distance over which a pattern of monophyly is expected
to be observed... Another extension would be to implement an approach
for the multispecies network coalescent. ... Waiting distances between
topology changes in genomic regions derived from one network history 
versus another may be highly informative about inferring local ancestry, 
or for inferring phylogenetic networks...}
...
of unlinked genealogies given a parameterized species tree, and is the basis
for modern phylogenetic inference of multi-locus datasets. However, because 
linkage among genealogies can cause their distribution to deviate from the 
expectations of an MSC model, multi-locus datasets are often highly filtered to
avoid this type of model violation. For example, even as whole genome sequence data 
has become increasingly available, a common practice has developed in which
a small number of discrete unlinked loci is extracted from whole genome 
...
In theory, a locus should correspond to a single genealogical tree or topology
across its entire length, depending on whether the MSC inference method 
uses branch length information or not. To the extent a locus corresponds to
sequences evolved on multiple distinct genealogies, a gene tree inferred from
that locus will be influenced by concatenation, which can bias the distribution
of gene trees to deviate from the expectations of an MSC model. This bias is
termed concatalescence \citep{liu2010maximum,gatesy_concatenation_2013,zhang2018astral}. 
... The waiting distance between topology-change events could provide a 
guide as to the appropriate size of markers to use for unlinked locus analyses.


% \subsection{Future directions in spatial phylogenetics}
We see the MS-SMC' as a step towards a spatially-explicit 
species tree inference framework. One in which recombination events
are viewed as a source of information as opposed to a source of
error. Many challenges await. Not least being the fact that we cannot
observe genealogies, and so the approach described here must be 
extended to observed SNP patterns in genomes. In this way, we see 

the approach of SNP-based methods like SNAPP as a likely precursor to 
future methods. One where rather than samplign unlnked SNPs, we could 
use any observed SNP patterns, and the genomic distances between them, 
to model not only the distributions of genealogies that could produce
each SNP, but also the probability that genealogies underlying those
SNP patterns would exist at that spatial distance apart. ...
...
\subsection{better-title-Sliding windows and local ancestry inference}
