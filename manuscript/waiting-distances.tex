...
\begin{figure}
	...
	\caption{
		...
		rates and the number of samples present, such that a table can be composed of
		...
		MSC model parameters in Table~S=2.
...
\begin{figure}[h]
	...
	\caption{
		...
		genomic positions, and which are separated by recombination breakpoints. 
		In this example four recombination breakpoints occurred between the 
		starting genealogy (left) and the final genealogy (right). Recombination
		events are indicated by panels which show the process by which a lineage
		is detached (former section shown as blue dotted line) and then 
		re-coalesces (red) with the other samples. Vertical bars (white) 
		act as barriers preventing coalescence between samples in different
		species tree intervals.
		% ancestor is pruned (blue dotted) 
		% and replaced by a new sampled ancestor (red).
		This process can result is one of four categories of outcomes:
...
which we refer to as the genealogy embedding 
...
These variables,
and the genealogy embedding table, include all relevant parameters
