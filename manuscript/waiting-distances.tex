...
In theory, a locus should correspond to sequences evolved on only a single 
...
application of the MS-SMC' is in serving as a guide for selecting 
appropriate locus lengths, where the average waiting distance between
...
direction. As an example, we showed that species tree parameters can be 
estimated from the information in genealogical waiting distances alone. 
However, many challenges would need to be overcome before this approach 
could become practical, not least of which is the fact that genealogies
...
aim to calculate the likelihood that any two SNP patterns, and thus
their underlying topologies, would exist given the spatial distance
that separates them. 
