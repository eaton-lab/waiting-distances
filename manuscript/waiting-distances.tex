...
We refer to this as a genealogy embedding table (e.g., Table~\ref{tab:table-1}). 
...
$k$ and $N_e$ in any species tree interval. 
...
we can get the probability of a topology-change event. 
As in the first section, we will first derive a solution for an individual 
branch and time of recombination, then for an entire branch, and
finally sum across branches to get a probability for an entire genealogy. 
To isolate events that do not cause a topology change requires finding
the union of events that cause a no-change event in addition to two 
possible tree-change events which affect only branch lengths but not the 
topology. These two events correspond to a re-coalescence with the 
sister to branch $b$, referred to as $b'$, or with its parent, referred to 
as $c$. Because the branch $c$ is always ancestral to $b$, a recombination
...
possible, which we term $t_b^m$. 

In the single population model of \citet{deng_distribution_2021} 
$t_b^m$ occurs at the maximum value of $t_b^l$ and $t_{b'}^l$, 
representing the lower bounds of $b$ and $b'$. 
In an MSC model we must incorporate additional possible constraints 
imposed by population barriers, and so $t_b^m$ occurs the maximum
of $t_b^l$, $t_{b'}^l$, 

this time point as the maximum value of three potential time points, 
to 
points are the lower bound of $b'$ (Fig.~\ref{fig:figS-tbm}b), 
the 
is more complex since population barriers can impose additional 
constraints. Instead, we use the maximum value of $t_b^l$, $t_{b,b'}^w$, 
and $t_{b'}^l$ 

... Define $\mathcal{M}$ and $\mathcal{L}$. 
...
as a two-part solution. ALSO DO EQUAL TO. 
If $t_r$ > $t_b^m$, then re-coalescence can occur
in three distinct regions, .... Alternatively, if $t_r$ < $t_b^m$, then 
re-coalescence can only occur in x or y. 
