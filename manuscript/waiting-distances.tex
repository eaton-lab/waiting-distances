...
Our \emph{ipcoal} package also includes a coalescent simulation and validation 
...
function of the where and when it occurs.
...
C and D. 
As an demonstration, we show probabilities of recombination outcomes for 
three selected branches of the genealogy (Fig.~\ref{fig:fig1}). 
Note that the probability of no-change and tree-change events are inversely
related (they sum to 1), since one or the other must happen as a consequence 
...
where a branch re-coalesces with a branch other than itself, 
