...
tree-change event, such that one or the other must always occur.
% , resulting in either 
...
Then, from the law of total probability we calculate the probability
of a tree-change. 
Finally, we calculate the probability of topology-change as a subset of
the probability of a tree-change, in which we restrict which branches the
detached lineage re-coalesces with.
% events. 
...
whereas the later term describes the probability of re-connecting in a different interval
on the same branch (i.e., $ij$), where $i$ is the interval index in which $t_r$ occurs. 
...
events, and we now know its probability, we can modify this equation 
to yield the probability density of a no-change event. 
Here, distances continue to be exponentially distributed, however, 
the new rate parameter ($\lambda_n$) is reduced proportionally by 
...
of a no-change event, its probability is simple to calculate, 
leading to a rate parameter ($\lambda_g$) for a probability 
density of a tree-change event.
...
a topology-unchanged event, where topology-unchanged represents the sum 
...
\begin{equation}
	\mathbb{P}(\textrm{topology-unchanged} | \mathcal{S},\mathcal{G},b,t_r)=
	\begin{dcases}
		\frac{1}{a_i} + \sum_{j=i}^{\mathcal{I}_{bc}-1}P_{ij}\exp\left\{\frac{a_i}{2n_i}t_r\right\} + 
		\sum_{j=m}^{\mathcal{I}_b-1}P_{ij}\exp\left\{\frac{a_i}{2n_i}t_r\right\}, 
		& \text{if } t_r < t_b^m \\
		\\
		2\bigg(
...
of waiting distances between recombination events (equation 7), 
no-change events (equation 9), and tree-change events (equation 10), 
a rate parameter ($\lambda_t$) can be calculated using equation 13 
to get the probability density of waiting distances between 
topology-change events, given a genealogy and an MSC model:
...
tree-change, the solution for a topology-change represents an
approximation. This is because we have ignored the possible effects of 
intermediate recombination events that cause tree changes but 
not topology changes during the waiting distance until a topology-change
...
demonstrated that its effect can likely be ignored. This is because of the 
...
Given a species tree model and initial genealogy the probabilities of different 
types of outcomes of recombination can be visualized as a function of when and 
where recombination occurs. This is demonstrated on an imbalanced 4-tip species 
tree with variable effective population sizes across branches 
(see parameters in Table~S2), and with a genealogy of seven samples embedded, 
including three from lineage A, two from lineage B, and one from lineages
C and D (Fig.~\ref{fig:fig1}a). 
As a demonstration, the probabilities of no-change, tree-change, and topology-change
are plotted along the lengths of three selected branches on the genealogy 
(Fig.~\ref{fig:fig1}b-d). 
Note that the probability of no-change and tree-change events are inversely
related, and sum to 1, since one or the other must occur as a consequence 
...
single-population model \citep{deng_distribution_2021}.
...
tree and topology-change events. Of particular interest in the 
...
This is true more generally as well: the effective population size across 
intervals affects the probabilities of re-coalescence, and thus has either
a smoothing (high Ne) or accentuating (low Ne) effect on how local
probabilities within each interval affect broader probabilities across a branch
(Fig.~S\ref{fig:figS-edge-probabilities}).
...
To validate that our analytical solutions for waiting distances are correct we 
compare our results to measured waiting distances between genealogies
from coalescent simulations. This was tested for species trees of different numbers
of tips; different topologies; different numbers of samples from each lineage; 
...
distances (Fig.~supplement-X). 

% reference results in figure 4.
Waiting distance results for a 10-tip balanced species tree with a 
crown height of 1M generations, equal internal edge lengths, and a per-site
per-generation recombination rate of 2e-9 is shown 
for 10 different constant Ne values (Fig.~\ref{xxx}a).
...
\begin{figure}[t]
	...
	\caption{
		...
		among which a genealogy for 8 samples is embedded.
	}
...
\begin{figure}[p]
	...
	\caption{brief older description.}
...
\begin{figure}[p]
	...
	\caption{\hl{maybe remove the stuff on the left.}}
