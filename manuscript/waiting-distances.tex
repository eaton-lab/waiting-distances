...
on the probability of topology-change. In the next section we validate 
...
Here we also use a coalescent simulation framework in \emph{ipcoal}, 
based on \emph{msprime} \citep{baumdicker_efficient_2022}
and \emph{toytree} \citep{eaton_toytree_2020}, for setting up MSC
...
% position (time) on a branch where recombination occurs. 
...
the probabilities of different recombination event outcomes represent
...
interest is the large different in probabilities of tree-change versus 
topology-change from intervals from $W_{AB}$ to $W_{ABC}$ (Fig.~\ref{fig:fig2}c-d).
The probability of topology-change decreases much faster than the probability of 
tree-change as the recombination event occurs closer to $t_9$. This is because 
following $t_9$ there is a large stretch of time during which re-coalescence can
only occur with the same branch or its sibling, which result only in tree-change
...
% summary of this sections first goal
