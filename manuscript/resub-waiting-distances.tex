...
\begin{figure}[p]
	...
	\caption{
		\textcolor{red}{
		Error in expected waiting distance to tree or topology-change events calculated
		under the MS-SMC. 
		Error was measured as (observed - expected) / expected, where observed is the 
		spanning distance of the first genealogy in a tree sequence until a tree or 
		topology-change event, and expected is the predicted waiting distance of that
		genealogy given its embedding in the species tree model.
		Tree sequences were simulated for different demographic models across a range 
		of parameter settings for Ne, and under two different ancestry models
...
this bias is not observed in the distance to tree-change events suggests
that the effect of the SMC' approximation has little effect on the 
calculations for an individual genealogy, but that these effects may 
compound when applied to multiple tree-change events that occur between
topology-change events. In other words, the SMC' introduces little bias
to waiting distance estimations by itself, but can interact with a second
source of bias -- the inhomogeneity of trees between topology-change events,
discussed below -- to cause a more significant bias that only affects
estimations of topology-change waiting distances.


% closely with the results of simulationsn
% \sout{
% showed that predicted waiting distances estimated under 
...
% (Fig.~\ref{fig:figS-bias-smc}c), a scenario with very little genealogical
% discordance.
% }
