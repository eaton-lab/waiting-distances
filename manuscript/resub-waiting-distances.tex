...
the computed event probabilities, $r$, and $L(\mathcal{G})$ of the starting tree.
}
...
$L(\mathcal{G})$ higher 
\textcolor{red}{across all values of Ne}
...
This is most clear at low values of Ne, where shorter coalescent times are
...
relative to tree or topology-change events. By contrast, when Ne is high the 
...
"species tree" demographic model scenario at very low Ne values -- a 
...
of genomes sampled per lineage in demographic models.
\sout{
...
tances in models with low Ne. Future work can likely find a correction for this bias.
}
}
...
\textcolor{red}{
\subsection{\sout{MS-SMC’ likelihood inference framework}}
\subsection{\textcolor{red}{A Likelihood Framework for Modeling Waiting Distances}}
\sout{
...
\begin{figure}[p]
	...
	\caption{
		...
		2-population demographic model with four MSC parameters (Ne0=200K, Ne1=300K, 
		Ne2=400K, T=1M) and recombination rate $r$=2 x 10$^{-9}$. 
...
variable $N_e$ (Ne0=200K, Ne1=300K, and Ne2=400K), a divergence time
...
The log-likelihood surfaces show that the waiting distance information
in an ARG can provide significant information for estimating MSC model 
parameters when analyzed under our MS-SMC model 
(Fig.~\ref{fig:fig-likelihood-posterior}). 
...
(Fig.~\ref{fig:fig-likelihood-posterior}a-c).
...
high uncertainty and a potential correlation structure. However, their
...
(Fig.~\ref{fig:fig-likelihood-posterior}a-b).
...
(Fig.~\ref{fig:fig-likelihood-posterior}d). 
Here, the waiting distance data is a poor fit to the model, providing
inaccurate estimates of both $r$ and Ne0, especially from topology-change 
...
surfaces, we proceeded to fit a full MSC model to jointly estimate all five MSC
...
using U(1e5, 1e6) for Ne parameters, U(1e5, 2e6) for W, and U(1e-9, 3e-9) for $r$.
Four separate MCMC chains were run on the same waiting distance dataset, each 
with a different random seed to start the MCMC sampler. We used the same dataset
as above, and calculated the combined likelihood of both tree-change and 
topology-change waiting distances.
...
variable Ne (Fig.~\ref{fig:fig-likelihood-posterior}a), we recovered accurate
estimates for all three Ne parameters, as well as the divergence time and 
recombination rate (Fig.~\ref{fig:fig-likelihood-posterior}b). The true simulated
values fall within the 95\% highest posterior density (HPD) for all parameters. 
...
Unlike for waiting distances, we could not combine the log-likelihoods 
from both sets of trees, as it would represent double-counting.
The likelihood of each tree was weighted by the length of sequence that it spanned.
The log-likelihood surface of Ne parameters under the MSC model was less 
tightly concentrated near the true values than the MS-SMC model, but 
exhibited a more round shape, suggesting low uncertainty and minimal 
correlation between parameters 
(Fig.~\ref{figS:fig-MSC-SMC-surfaces}a-b). 
The summed log-likelihood of the genealogy data under the MSC was 
approximately three orders of magnitude greater than from waiting 
distance data under the MS-SMC, 
reflecting that there is generally much more information in the coalescent 
times of a tree than in the interval length that it spans.
% 
...
(Fig.~\ref{figS:fig-MSC-SMC-surfaces}c). The combined likelihood surfaces 
based on both MSC and MS-SMC data show a distinct peak closer to the 
true values than under the MSC model alone, and exhibit a much rounder 
shape with less correlation structure than under the MS-SMC model alone.
% 
% 
We next applied our Bayesian joint inference framework to fit the full 
6-parameter model under the MSC or the MSC + MS-SMC likelihoods. In the
latter, we summed the MSC log-likelihoods of genealogies between tree-change
events and the MS-SMC log-likelihoods of both tree-change and topology-change
waiting distances. Each analysis was run with four chains that were checked
for convergence and then combined, as above.
The marginal posterior distributions for each parameter inferred from the 
MSC model were more precise than under the MS-SMC, except for recombination rate,
for which the MSC model provides no information, and thus returned the uniform prior
(Fig.~\ref{figS:fig-likelihood-posterior}b).
The marginal posterior distributions inferred from the combined MSC + MS-SMC models
was the most accurate, providing precise estimates for the demographic model 
parameters and recombination rate
(Fig.~\ref{figS:fig-likelihood-posterior}c).
This suggests that a combined likelihood framework using both the MSC to
calculate the likelihood of genealogies, and the MS-SMC to calculate the 
likelihood of one or more spanning distances, can serve as an accurate 
statistical framework for analyzing ARG data as a function of parameters
of a structured demographic model, such as a species tree.
...
\textcolor{red}{While previous work has explored linkage in a species tree 
...
lay a groundwork for exploring how variation in
...
% MOVE TO DISCUSSION --- 
While it has long since been shown that the parameters of an MSC model can
be estimated from the coalescent times on genealogies, here we have shown
for the first time that they can also be estimated from the lengths of 
the intervals that genealogies span in the genome between recombination events
causing a tree-change or topology-change. This begs the questions of which
is more informative, and whether these two information sources can be combined.
Each genealogy within an interval of the genome can contain many coalescent
times, but only one topology-change event
% 
and thus it is not surprising that the waiting times to coalescent events
are much more informative than the waiting distances to recombination events.
% 
ent model can be fit from the coalescent
Given that MSC parameters 
This raises the final important question of where and when waiting distance
information is most useful.
- Waiting distances have been under-explored as a source of historical information.
- Here, we showed that they are data rich.
- However, our computations require an estimate of the genealogy within each interval,
and so it would be odd to not also use information on the coalescent times.
- We computed likelihood surfaces on the same dataset from above, but now computed 
likelihoods of the genealogies in each interval under the MSC model. 
- The likelihood of each tree was scaled by the spanning length of that tree.
- These likelihood surfaces are similar to those observed from waiting distances
in Fig. X, but are often of different shape. 
- Finally, we examined likelihood surfaces for 
% 
Historically, waiting distance information has provided  in an ARG represent 
...
modeled and visualized (Fig.~\ref{fig:edge-probabilities}; Fig.~\ref{fig:figS-edge-probabilities}), and 
...
\textcolor{red}{There is an ongoing conversation in the literature 
about the impact of concatalescence on inference under the MSC, and studies have used
simulations with intra-locus recombination to investigate this \citep{lanier2012recombination,zhu2022simulation}. 
...
\textcolor{red}{Doing so avoids biases similar to what we observe 
in Fig.~\ref{fig:fig-likelihood}e, making ARGweaver-D a promising starting point for 
...
\sout{estimated} \textcolor{red}{calculated} 
...
\begin{figure}[p]
	...
	\caption{
		\textcolor{red}{
		Error in MS-SMC waiting distance expectations caused by the SMC' approximation.
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.99\textwidth]{figures/current/FigS-bias-fold-topo-first.pdf}
	\caption{
		\textcolor{red}{
		...
		}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.99\textwidth]{figures/current/FigS-bias-fold-topo-last.pdf}
	\caption{
		\textcolor{red}{
		...
		}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.99\textwidth]{figures/current/FigS-MSC-SMC-surfaces.pdf}
	\caption{
		...
	}
     \label{fig:figS-MSC-SMC-surfaces}
\end{figure}
...
we repeated the simulations from our validation scenario using the 
\emph{msprime} setting ancestry="smc\_prime" to simulate tree sequences 
while excluding recombination events that would not occur in the SMC' model.
% 
\textcolor{red}{
Our validation analysis showed that predicted waiting distances estimated under 
the MS-SMC align very closely with results of simulations, even when the simulations
are performed under the full coalescent with recombination model
(Fig.~\ref{fig:fig-validation}).
The percent error in these analyses was measured as 100 $\times$ (simulated waiting distance
- expected waiting distance) / expected waiting distance. Regardless
of whether data were simulated under the full or SMC' model, the percent error
in waiting distance estimations rarely exceeded 2\% (Fig.~\ref{fig:figS-bias-smc}).
}
% 
% Less error was observed for tree-change waiting
\textcolor{red}{
The error is consistently lower for tree-change than for topology-change 
distances, and only exceeded 5\% for topology-change waiting distances 
calculated in the "Phylogeny Model" at low $N_e$ values
(Fig.~\ref{fig:figS-bias-smc}c), where it reached up to 
}
% 
\textcolor{red}{
The error rate observed in this scenario is interesting in three respects: 
...
% ADDED
We predicted that the SMC' would tend to exhibit greater waiting distances
until a tree or topology change event than the full model, and that this 
would be increased at low $N_e$ values, where 

. However, we
did not observe a significant directional error. Regardless of the demographic model examined, and 
its parameters, the error in tree-change waiting distances was not 
significantly different when the data were simulated under the SMC' versus
full coalescent with recombination models (Fig.~\ref{fig:figS-bias-smc}a-d).
Notably, we do see a significantly greater error arise ...
