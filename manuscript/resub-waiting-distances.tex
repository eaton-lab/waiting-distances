...
regions. 
% 
\st{Expectations for the amount of g}
% 
\textcolor{red}{G}enealogical variation among unlinked 
...
%
\st{the expected distribution of waiting distances between changes in genealogical trees} 
%
\textcolor{red}{the distribution of the waiting distance for a change in the genealogical tree} 
...
result, in terms of the \st{expected} distribution of waiting distances between 
changes in genealogical trees and topologies\st{,} for multiple structured populations
with branch-specific effective population sizes (i.e., under the multispecies 
coalescent). 
\textcolor{red}{\st{Our solutions establish an expectation for genetic linkage 
...
% by providing a likelihood equation for an ARG
% in the presence of population structure.}} 
\textcolor{red}{We show that tree and topology waiting distances can be used in a
novel likelihood framework to compare ARGs given a parameterized 
species tree, as well as to infer species tree parameters from 
one or more ARGs, thus opening multiple pathways for future development.
The multi-species sequentially Markov coalescent (MS-SMC) presented here
represents a major advance for both local ancestry inference and 
recombination-aware species tree inference.}
% can be used to link demographic models with local ancestry inference across genomes.
% }}

% ARG in multi-population datasets, providing a novel framework for 
% extracting spatial genealogical information to link demographic models
% with local ancestry inference across genomes.

	

% . This new framework links demographic
% models with local ancestry inference across genomes.

%  and extend the framework for calculating the 
% likelihood ofARG
% likelihood

% provide a new likelihood framework for linking
% demographic models with local ancestry inference across genomes.
...
% (Fig.~\ref{fig:fig1}a)
\textcolor{red}{\st{(Fig.~1a)}}, and the joint likelihood of 
...
Importantly, 
\textcolor{red}{\st{both the coalescent and MSC are models of the expected distribution of 
\emph{unlinked} (uncorrelated) genealogies.}}
\textcolor{red}{both the single-population coalescent and the MSC specify 
...
\textcolor{red}{To be clear, the spatial correlation here refers to the similarity 
...
\st{An algorithm to stochastically simulate sequences under the coalescent with 
...
\textcolor{red}{An algorithm to stochastically generate ARGs under the coalescent with recombination was developed early on by \citet{hudson1983properties} and later extended as a spatial algorithm by \citet{wiuf_recombination_1999}.}
This \textcolor{red}{latter} process models the difference between sequential genealogies by 
randomly detaching an edge from a genealogy and sampling
a waiting time (based on the 
\textcolor{red}{\st{population}}
coalescent rate) until it reconnects
to the genealogy at a different shared ancestor.
...
the \st{expected} \textcolor{red}{distribution of} waiting distances to each of these three types of 
...
Here, 
\st{we extend the methods of \mbox{\citet{deng_distribution_2021}} to an MSC framework 
to estimate the expected waiting distances between different types of}
\textcolor{red}{we extend the theory of \citet{deng_distribution_2021} to an MSC framework
...
\textcolor{red}{
In addition, we demonstrte a novel likelihood implementation that makes
...
\st{likelihood solutions} \textcolor{red}{likelihood-based solutions} 
...
\st{with an expectation 
determined by the number of samples and the coalescent rate, 
similar to equation 1.}
\textcolor{red}{using equation (1), where the expectation is 
determined by the number of  samples and the coalescent rate.}
...
\textcolor{red}{(Fig.~\ref{fig:embedding-table}).}
% 
\textcolor{red}{\st{(Table~1).}}
% 
...
\begin{figure}[t]
	...
	\caption{
		...
		the resulting detached subtree re-coalescing with other available
		branches above that event.}
		\textcolor{red}{\st{
		...
		%, or $\mathcal{J}_b(i)$. 
		(b) Indexing variables for calculating \st{tree}\textcolor{red}{no}-change probabilities. The indexing variable $\mathcal{I}_b$ records all
		intervals on branch $b$\textcolor{red}{; \st{, and}}
...
\textcolor{red}{We provide a didactic example of this
...
\begin{figure}
	...
	\caption{
		...
		genealogy embedding table (see Table 1 \textcolor{red}{and Fig.~S2}).
		...
		are shown for three arbitrary \textcolor{red}{genealogy} branches. These probabilities are a function of the time ($t_r$) and \textcolor{red}{genealogy} branch 
...
\textcolor{red}{(See Fig.~\ref{fig:figS-edge-probabilities} for different 
parameterizations of this simulation.)}
...
\textcolor{red}{\st{end}} of a \textcolor{red}{gene tree} branch (further back in time). 
...
behave monotonically \textcolor{red}{\st{along the length of a branch} with respect
...
distances calculated under the MS-SMC are accurate, and contain significant
information for inferring population structure and demographic parameters.
}
\textcolor{red}{\st{
...
\textcolor{red}{\st{under-estimation}}
...
% \subsection{\textcolor{red}{Waiting Distances in Phylogenetics}}
...
The MS-SMC model provides the first statistical framework for comparing the fit
of different parameterized species tree models given one or more observed
ARGs. We investigated the use of different addition of waiting distance information from
ARG

% describe Fig. 6 
- The MSC model is widely used to fit demographic models based on unlinked
genome data (SNPs or gene trees) [review sources], such as when inferring
species trees, delimiting species, inferring divergence times, and 


Inferring species trees involves  is important.
(1) Currently a species tree can only be inferred from unlinked genealogical
data 
...
\textcolor{red}{
Here we demonstrate an application of MS-SMC theory to provide a new 
and more informative framework for calculating the likelihood of an
ARG. 
In contrast to the current approach which models the waiting distances
between each recombination event 
regardless of its event type \citep[e.g.,][]{wiuf_ancestry_1999, kuhner_maximum_2000}, 
and without the ability 
Fig.~\ref{fig:figS-bias-smc}
our approach extracts significantly more information from the spatial
distribution of genealogies by incorporating the probabilities of observing
several categorical types of recombination events, which span over varying
...
\textcolor{red}{rudimentary} maximum 
likelihood framework \textcolor{red}{as a proof-of-concept} for the latter: 
we apply the MS-SMC to fit MSC 
...
\begin{figure}[t]
	...
	\caption{
		The MS-SMC likelihood framework. (a) An ARG is composed of a sequence of
		trees and interval lengths separated by recombination events. (b-c) By delimiting
		intervals by the type of event (contains three nested sets of 
	}
...
\st{These solutions} \textcolor{red}{Our generalized solutions here} 
...
Differences between the \st{expected} waiting distances in genomic regions derived from 
...
\st{Applications of ARG inference have historically been applied at the
...
\st{However,} ARGs can exist for any stretch of the genome, 
...
the recent method ARGweaver-D} \st{and recent methods} can 
...
\st{estimated} \textcolor{red}{calculated} 
...
\st{A theoretical ARG-based extension of this approach might 
...
\begin{figure}[p]
	...
	\caption{
		...
		with recombination (green). The MS-SMC tends to 
		\st{under}\textcolor{red}{over}-estimate 
...
Examples showing how to compute the \textcolor{red}{probability}\st{probablity} of a no-change (tree-unchanged) 
or topology-\textcolor{red}{unchanged}\st{change} event are shown with didactic step-by-step instructions in 
